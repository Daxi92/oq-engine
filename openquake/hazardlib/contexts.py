    ...
    def get_pmap_by_grp(self, srcfilter, group):
        ...
        while True:
            ...
            try:
                ...
                poemap = pmaker.make(srcs, sites, pmap, rup_data)
            except StopIteration:
                break
            totrups += poemap.totrups
            calc_times[srcs[0].id] += numpy.array(
...
class PmapMaker(object):
    ...
    def make(self, srcs, sites, pmap, rup_data):
        """
        :param srcs: a list of hazardlib sources affecting the same sites
        :param sites: the sites
        :returns: the probability map generated by the source
        """
        with self.cmaker.mon('iter_ruptures', measuremem=False):
            rups = sum([list(src.iter_ruptures(shift_hypo=self.shift_hypo))
                        for src in srcs], [])
            self.mag_rups = groupby(rups, operator.attrgetter('mag')).items()
